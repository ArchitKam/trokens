...
def load_vid_reader(vid_path, start_sec, end_sec, return_tensor=False,
                      use_float=False, num_frames=8,
                      sample_all_frames=False, fps=None, device=None):
    '''
    load video from file with regular interval sampling using video_reader.
    Args:
    ...
    print('finished loading video, moving on')
...
def load_video_decord(vid_path, return_tensor=False,
    ...
    print("AV failed, trying decord")
    vr = PyVideoReader(vid_path, num_threads=1)
